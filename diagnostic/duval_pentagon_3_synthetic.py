# -*- coding: utf-8 -*-
"""duval_pentagon_3_synthetic.py

This module calculates duval pentagon 3 (synthetic) related diagnostics and generates duval pentagon visualizations using plotly library.
...
## Defining Duval pentagon 3 (synthetic) coordinates and areas

# D2 - Discharges of high energy
D2x = [4, 0, 0, 24.204, 32.061, 4]
D2y = [16, 1.5, -3, -30.229, -6.048, 16]
...
def create_duval_p3_synthetic_colorized(legend_show=False):
    fig = go.Figure()
    ...
    fig.add_scatter(x=[-16, -0.5, 16, 14, 7, -8, -20], 
                    y=[16, 28.75, 16 ,-5, -20, -20, -8],
...
def create_duval_p3_synthetic_nocolor(legend_show=False):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=Sx, y=Sy, 
                             name='S',
                            showlegend=legend_show,
...
def calculate_duval_p3_synthetic_coordinates(h2, c2h6, ch4, c2h4, c2h2, rounding=False):
...
def calculate_duval_p3_synthetic_result(h2, c2h6, ch4, c2h4, c2h2):
    ...
    try:
        ...
        if ((h2 == 0) and (c2h6 == 0) and (ch4 == 0) and (c2h4 == 0) and (c2h2 == 0)) is True:
            ...
            centroid_list, summit_list = calculate_duval_p3_synthetic_coordinates(h2, ch4, c2h6, c2h4, c2h2)
...
def create_duval_p3_synthetic_marker(h2, c2h6, ch4, c2h4, c2h2, **kwargs):
    marker_coord, summit_list = calculate_duval_p3_synthetic_coordinates(h2, c2h6, ch4, c2h4, c2h2)
    result = calculate_duval_p3_synthetic_result(h2, c2h6, ch4, c2h4, c2h2)
...
def draw_duval_p3_synthetic_summits(h2, c2h6, ch4, c2h4, c2h2):
    marker_coord, summit_list = calculate_duval_p3_synthetic_coordinates(h2, c2h6, ch4, c2h4, c2h2)
...
def create_duval_p3_synthetic_result_graph(h2, c2h6, ch4, c2h4, c2h2, include_summit=False):
    fig = create_duval_p3_synthetic_colorized()

    try:
        result_name = calculate_duval_p3_synthetic_result(h2, c2h6, ch4, c2h4, c2h2)
        fig.add_trace(create_duval_p3_synthetic_marker(h2, c2h6, ch4, c2h4, c2h2, result_name))
        if include_summit is True:
            fig.add_trace(draw_duval_p3_synthetic_summits(h2, c2h6, ch4, c2h4, c2h2))
        return fig
    except:
        return fig
    
def create_duval_p3_synthetic_multi_results_graph(samples_df):
    fig = create_duval_p3_synthetic_colorized()
    ...
    try:
        ...
        for row in samples_df.itertuples(name=None):
            ...
            if ((h2 == 0) and (ch4 == 0) and (c2h6 == 0) and (c2h4 == 0) and (c2h2 == 0)) is True:
                ...
                duval_result = calculate_duval_p3_synthetic_result(h2, c2h6, ch4, c2h4, c2h2)
                mark_name = f'{duval_result} {time}'
                fig.add_trace(create_duval_p3_synthetic_marker(h2, c2h6, ch4, c2h4, c2h2, mark_name, timestamp=time, result=duval_result, marker_color=rowcolor))
        return fig
    except Exception as e:
        print(e)
        return fig
...
if __name__ == "__main__":
    ...
    dp3_synthetic_coord, dp3_synthetic_summits = calculate_duval_p3_synthetic_coordinates(31, 130, 192, 31, 0)
    print(f'centroid_XY:\n{dp3_synthetic_coord}\nsummit_coordsXY:\n{dp3_synthetic_summits}')

    poly1 = Polygon(dp3_synthetic_summits)
    print(f'shapely_XY:\n{list(poly1.centroid.coords)}')

    dp3_synthetic_result = calculate_duval_p3_synthetic_result(31, 130, 192, 31, 0)
    print(dp3_synthetic_result)
    '''
    duvp3_synthetic_fig = create_duval_p3_synthetic_result_graph(31, 130, 192, 31, 0, include_summit=False)
    duvp3_synthetic_fig.show()
    '''
    # H2 = 50 ppm, C2H6 = 80 ppm, CH4 = 120 ppm, C2H4 = 60 ppm and C2H2 = 30 ppm  -> xo = -7.35, and yo = -5.79 (T1)
    # 50, 120, 80, 60, 30
    dp3_synthetic_coord2, dp3_synthetic_summits2 = calculate_duval_p3_synthetic_coordinates(50, 80, 120, 60, 30)
    print(f'centroid_XY:{dp3_synthetic_coord2}\nsummit_coordsXY:{dp3_synthetic_summits2}')

    poly2 = Polygon(dp3_synthetic_summits2)
    print(f'shapely_XY:{list(poly2.centroid.coords)}')

    dp3_synthetic_result2 = calculate_duval_p3_synthetic_result(50, 80, 120, 60, 30)
    print(dp3_synthetic_result2)
    '''

    duvp3_synthetic_2_fig = create_duval_p3_synthetic_result_graph(50, 80, 120, 60, 30, include_summit=False)
    duvp3_synthetic_2_fig = create_duval_p3_synthetic_nocolor()
    duvp3_synthetic_2_fig.add_trace(create_duval_p3_synthetic_marker(50, 80, 120, 60, 30, timestamp='2021-05-11', marker_color='blue'))
    #duvp3_synthetic_2_fig.add_trace(draw_duval_p3_synthetic_summits(50, 80, 120, 60, 30))
    duvp3_synthetic_2_fig.show()
    ...
    #duvp3_synthetic_multi_fig = create_duval_p3_synthetic_multi_results_graph(df_sample)
    #duvp3_synthetic_multi_fig.show()
